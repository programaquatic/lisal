...
impl MeshOfSquares {
    ...
    */
    pub fn new(area_size: UVec2, pos_scale: Vec2, uv_scale: Vec2) -> MeshOfSquares {
        ...
        (0..area_size.y)
            .cartesian_product(0..area_size.x)
            .for_each(|(y, x)| {
                // texture coordinates
                let uv = Vec2 {
                    x: x as f32,
                ...
                let pos = Vec3 {
                    x: x as f32 * pos_scale.x,
                    y: 0.0,
                    z: y as f32 * pos_scale.y,
                };
                locations.push(pos);
                normals.push(Vec3 {
                    x: 0.0,
    ...
    pub fn randomize_position(mut self, range: (f32, f32)) -> MeshOfSquares {
        ...
        for vertex in self.locations.iter_mut() {
            let new_y =
                rng.random_range(range.0 * granularity..range.1 * granularity) / (2. * granularity);
            vertex.y = new_y;
        }
        self
    }
    ...
    pub fn randomize_normals(mut self, noise_level: f32) -> MeshOfSquares {
        ...
        for normal in self.normals.iter_mut() {
            normal.x +=
                rng.random_range(-granularity / 2.0..granularity / 2.0) * (noise_level / 2.0);
            normal.y +=
                rng.random_range(-granularity / 2.0..granularity / 2.0) * (noise_level / 2.0);
            normal.z +=
                rng.random_range(-granularity / 2.0..granularity / 2.0) * (noise_level / 2.0);
